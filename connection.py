...
def save_blockchain():
    ...
    for j in my_data:
        ...
        if new_message[0] == "preRegistro":
            account_who_stores = int(new_message[1])
            prueba.pre_registro(account_who_stores, new_message[1], new_message[2], new_message[3])
    ...
    while True:
        ...
        try:
            if "ummisco.gama.network.common.CompositeGamaMessage" in msg:
                ...
                if spli[0] == "preRegistro":
                    prueba.pre_registro(int(spli[1]), spli[2], spli[3], spli[4])

                if spli[0] == "QR":
                    prueba.view_Pre_Registro(spli[1])
                ...
                if spli[0] == "Aplicar":
                    tokens += 1
                    print(tokens)
                    send_udp_message("priority1")

                if spli[0] == "priority2":
                    send_udp_message("priority2")
